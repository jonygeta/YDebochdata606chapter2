...
In a streak of length 1 there are 24  hits and 24 misses. In a streak of length 0 there are 0 hits and 39 misses.
...
barplot(table(kobe_streak))
```
His typical streak length was 0. His longest streak of baskets was 4.
## Compared to What?
...
4.  What change needs to be made to the `sample` function so that it reflects a 
    ...
    `sim_basket`.
Answer: We need to add prob value to 0.45,0.55  for shooting percentage of 45%.
```{r}
...
Answer: Typical streak length is 0 with a 45% shooting percentage. The longest streak length is 3.
...
Answer: If I ran the simulation of the independent shooter a second time I expect it's streak distribution will be almost similar to the somewhat similar. The reason for this is in both cases the shooting percentages are same but the sample drawn might be different.
...
Answer: Kobe Bryant's distribution of streak lengths is somewhat similar to independent shooters. Kobe Bryant's highest streak is 4 while independent shooters is 3. From the comparison independent shooters don't have highest streak length as Kobe. As kobe has more longer streaks than independent shooter we can conclude that hot hand model fits Kobe's shooting patterns.
<div id="license">
